 ...
 */
public class UserRegistApi implements Api {
    ...
    public JSONObject execute(JSONObject params) {
        ...
        try {
            String valid = params.optString("valid");
            ...
            // 已注册 用户存在且密码不为空
            if (userModel != null) {
                return errorResult(result, "您的手机号码已经注册", "错误：已注册用户。");
            }
    ...
    public UserModel addUser(String phone, String password) {
        UserModel userModel = new UserModel();
        userModel.setPhone(phone);
        userModel.setPassword(password);
        userModel.setCreateTime(new Date());
        userModel.setLastLoginTime(new Date());
        userModel.setLoginTime(userModel.getLoginTime() == null? 0: userModel.getLoginTime() + 1);
        boolean isSuccess = false;
        userService.addUser(userModel);
        isSuccess = userModel.getId() != null;
        if (isSuccess) {
            return userModel;
        } else {
            return null;
